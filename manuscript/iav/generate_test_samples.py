...
def main():
    ...
    parser.add_argument("--root_folder", required=True, help="Root folder containing the clade folders.")
    ...
    # clade parameters
    parser.add_argument("--coverage", type=float, default=100.0, help="Target fold coverage per clade.")
    # Read parameters
    ...
    parser.add_argument("--num_seqs", type=int, default=3, help="Number of sequences to select per clade.")
    ...
    for sub in sorted(root_folder.iterdir()):
        ...
        seqs = sorted(sub.glob("*.fasta"))
        if seqs:
            clades.append(sub.name)
            sequences_per_clade[sub.name] = seqs

    if not clades:
        raise ValueError(f"No clades found in root folder: {root_folder}")
    ...
    for clade_idx, clade in enumerate(clades):
        seq_files = sequences_per_clade[clade]

        # Selection per clade
        sequences_seed = int(hashlib.sha256(f"{sample_num}_{clade}_{root_folder.name}".encode("utf-8")).hexdigest(), 16) % (2**31 - 1) # create a stable 32bit seed per clade for selecting genomes
        ...
        k_sel = len(selected)
        cov_per_genome = args.coverage / float(k_sel) # equal coverage per genome in clade

        for g_idx, genome_path in enumerate(selected):
            # Create unique prefix
            prefix = f"s{sample_num:02d}_{clade}_g{g_idx+1}"
            ...
            reads_seed = int(hashlib.sha256(f"{sample_num}_{clade}_{root_folder.name}_{g_idx}_{genome_path.name}".encode("utf-8")).hexdigest(), 16) % (2**31 - 1) # create a stable 32bit seed per genome for ART
    ...
    with open(merged_r1, "wb") as wfd:
        for part in r1_parts:
            with open(part, "rb") as fd:
                shutil.copyfileobj(fd, wfd)
    with open(merged_r2, "wb") as wfd:
